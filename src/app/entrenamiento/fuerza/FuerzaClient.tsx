...
export function FuerzaClient() {
  ...
  return (
    <div>
      <StickyAgendar href={AGENDA_URL} color={ORANGE} />
      <section ref={heroRef} style={{ height: "320vh" }} className="relative">
        <div className="sticky top-0 h-screen min-h-[100svh] overflow-hidden bg-[#0A0A0A]" style={{ transform: "translateZ(0)" }}>
          <div ref={bgImgRef} className="absolute inset-0" style={{ transformOrigin: "50% 30%", willChange: "transform" }}>
            <Image src={`${BP}/fuerza4.webp`} alt="Fuerza IRONFIT" fill priority className="object-cover" style={{ objectPosition: "50% 45%" }} sizes="100vw" />
          </div>
          ...
          <div ref={p2Ref} className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center pointer-events-none" style={{ opacity: 0, transform: "translateY(60px)", willChange: "opacity, transform" }}>
            <span className="mb-5 rounded-full px-4 py-1.5 text-[11px] font-bold tracking-widest uppercase" style={{ backgroundColor: "rgba(249,115,22,0.2)", color: ORANGE, border: "1px solid rgba(249,115,22,0.4)" }}>Tecnología de precisión</span>
            <h2 className="text-4xl sm:text-5xl leading-[1.0] text-white md:text-7xl lg:text-8xl" style={{ fontFamily: "var(--font-display)", letterSpacing: "0.02em", textShadow: "0 2px 20px rgba(0,0,0,0.95), 0 4px 40px rgba(0,0,0,0.9)" }}>
          ...
          <div ref={p3Ref} className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center" style={{ opacity: 0, transform: "translateY(60px)", willChange: "opacity, transform" }}>
            <span className="mb-5 rounded-full px-4 py-1.5 text-[11px] font-bold tracking-widest uppercase" style={{ backgroundColor: "rgba(249,115,22,0.2)", color: ORANGE, border: "1px solid rgba(249,115,22,0.4)" }}>Planes desde $50.000</span>
            <h2 className="text-4xl sm:text-5xl leading-[1.0] text-white md:text-7xl lg:text-8xl" style={{ fontFamily: "var(--font-display)", letterSpacing: "0.02em", textShadow: "0 2px 20px rgba(0,0,0,0.95), 0 4px 40px rgba(0,0,0,0.9)" }}>
              Tu mejor versión<br /><span style={{ color: ORANGE }}>empieza hoy.</span>
            </h2>
            ...
            <motion.a href={AGENDA_URL} className="inline-flex items-center gap-2 rounded-2xl px-10 py-4 text-base font-bold text-white" style={{ backgroundColor: ORANGE }} whileHover={{ scale: 1.05, boxShadow: "0 0 48px rgba(249,115,22,0.6)" }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
        ...
        <div className="sticky top-0 h-screen min-h-[100svh] flex items-center overflow-hidden bg-[#0A0A0A]" style={{ transform: "translateZ(0)" }}>
              ...
              <ul className="mb-4 space-y-2.5">
                {[
                  ...
                  <li key={plan.clases} className="flex items-baseline justify-between gap-3 border-b border-white/10 pb-2 text-base">
                    ...
                    <span className="font-bold" style={{ color: ORANGE }}>{plan.precio}<span className="ml-0.5 text-xs font-normal" style={{ color: "rgba(255,255,255,0.4)" }}>/mes</span></span>
                  </li>
                ))}
              </ul>

              <div className="relative mb-4 overflow-hidden rounded-2xl px-5 py-4" style={{ background: "linear-gradient(135deg, rgba(249,115,22,0.20), rgba(249,115,22,0.05))", border: "1px solid rgba(249,115,22,0.5)", boxShadow: "0 0 44px rgba(249,115,22,0.20)" }}>
                <span className="mb-2.5 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-white" style={{ backgroundColor: ORANGE }}>★ Mejor valor</span>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-white/10 pt-3 text-sm">
                  ...
                  <span className="rounded-md px-2 py-0.5 text-xs font-bold" style={{ backgroundColor: "rgba(249,115,22,0.22)", color: ORANGE }}>Ahorras $50.000</span>
                </div>
              </div>

              <div className="mb-6 rounded-xl px-4 py-3 text-sm" style={{ backgroundColor: "rgba(249,115,22,0.08)", border: "1px solid rgba(249,115,22,0.25)", color: "rgba(255,255,255,0.85)" }}>
                <span className="font-bold" style={{ color: ORANGE }}>+ $20.000:</span> 3 sesiones de Recovery de 30 minutos.
              </div>
              <a href={AGENDA_URL} className="inline-flex items-center gap-2 rounded-2xl px-8 py-4 text-base font-bold text-white" style={{ backgroundColor: ORANGE, boxShadow: "0 0 32px rgba(249,115,22,0.35)" }}>Agendar Fuerza IRONFIT →</a>
            </div>
          </div>
          <div className="hidden lg:block relative h-full flex-1 overflow-hidden">
            <div ref={imgYRef} className="absolute inset-0" style={{ willChange: "transform" }}>
              <Image src={`${BP}/kettebell.webp`} alt="Entrenamiento con kettlebell" fill className="object-cover" sizes="50vw" />
            </div>
