  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (hero) {
        ...
        if (darkRef.current)  darkRef.current.style.opacity  = String(interp(sp, [0,0.15,0.55,1], [0.45,0.35,0.35,0.60]));
      ...
      <section ref={heroRef} style={{ height: "320vh" }} className="relative">
        <div className="sticky top-0 h-screen min-h-[100svh] overflow-hidden bg-[#0A0A0A]" style={{ transform: "translateZ(0)" }}>
          <div ref={bgImgRef} className="absolute inset-0" style={{ transformOrigin: "50% 50%", willChange: "transform" }}>
            <Image src={`${BP}/movilidad2.webp`} alt="Movilidad Articular" fill priority className="object-cover" sizes="100vw" />
          </div>
          ...
          <div ref={p1Ref} className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center pointer-events-none" style={{ opacity: 1, willChange: "opacity, transform" }}>
            <span className="mb-5 rounded-full px-5 py-2 text-xs font-bold tracking-widest uppercase" style={{ backgroundColor: "rgba(16,185,129,0.2)", color: GREEN, border: "1px solid rgba(16,185,129,0.4)" }}>Movilidad Articular</span>
            <h2 className="text-4xl sm:text-5xl leading-[1.0] text-white md:text-7xl lg:text-8xl" style={{ fontFamily: "var(--font-display)", letterSpacing: "0.02em", textShadow: "0 2px 20px rgba(0,0,0,0.95), 0 4px 40px rgba(0,0,0,0.9)" }}>
              Muévete sin<br /><span style={{ color: GREEN }}>dolor.</span>
            </h2>
          ...
          <div ref={p2Ref} className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center pointer-events-none" style={{ opacity: 0, transform: "translateY(60px)", willChange: "opacity, transform" }}>
            <span className="mb-5 rounded-full px-5 py-2 text-xs font-bold tracking-widest uppercase" style={{ backgroundColor: "rgba(16,185,129,0.2)", color: GREEN, border: "1px solid rgba(16,185,129,0.4)" }}>Método progresivo</span>
            <h2 className="text-4xl sm:text-5xl leading-[1.0] text-white md:text-7xl lg:text-8xl" style={{ fontFamily: "var(--font-display)", letterSpacing: "0.02em", textShadow: "0 2px 20px rgba(0,0,0,0.95), 0 4px 40px rgba(0,0,0,0.9)" }}>
              Recupera tu<br /><span style={{ color: GREEN }}>movimiento.</span>
            </h2>
            <p className="mt-6 max-w-[44ch] text-base leading-relaxed md:text-lg" style={{ color: "rgba(255,255,255,0.65)" }}>
              Movilidad articular · Control motor · Estabilidad funcional.<br />Cada sesión diseñada para tu cuerpo.
            </p>
          </div>
        ...
        <div className="sticky top-0 h-screen min-h-[100svh] flex items-center overflow-hidden bg-[#0A0A0A]" style={{ transform: "translateZ(0)" }}>
          <div className="relative z-10 flex h-full w-full lg:w-1/2 flex-col justify-center px-6 sm:px-10 md:px-16">
            ...
            <div ref={blk2Ref} className="absolute inset-x-6 sm:inset-x-10 md:inset-x-16" style={{ opacity: 0, willChange: "opacity, transform" }}>
              ...
              <div className="mb-4 flex items-start gap-3 rounded-xl px-4 py-3" style={{ backgroundColor: "rgba(16,185,129,0.10)", border: "1px solid rgba(16,185,129,0.30)" }}>
                <span className="mt-0.5 shrink-0 text-base font-bold" style={{ color: GREEN }}>✓</span>
                <p className="text-sm leading-snug" style={{ color: "rgba(255,255,255,0.85)" }}>
                  <span className="font-bold" style={{ color: GREEN }}>Evaluación InBody incluida.</span> Cada pack de 4 sesiones incluye una evaluación InBody para medir tu progreso — una aprox. cada 4 semanas, en todos los planes.
                </p>
              </div>
              ...
              <ul className="mb-4 space-y-2.5">
                {[
                  ...
                  <li key={plan.clases} className="flex items-baseline justify-between gap-3 border-b border-white/10 pb-2 text-base">
                    ...
                    <span className="font-bold" style={{ color: GREEN }}>{plan.precio}<span className="ml-0.5 text-xs font-normal" style={{ color: "rgba(255,255,255,0.4)" }}>/mes</span></span>
                  </li>
                ))}
              </ul>

              <div className="relative mb-4 overflow-hidden rounded-2xl px-5 py-4" style={{ background: "linear-gradient(135deg, rgba(16,185,129,0.20), rgba(16,185,129,0.05))", border: "1px solid rgba(16,185,129,0.5)", boxShadow: "0 0 44px rgba(16,185,129,0.20)" }}>
                <span className="mb-2.5 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest text-white" style={{ backgroundColor: GREEN }}>★ Mejor valor</span>
                <div className="flex items-end justify-between gap-3">
                  ...
                  <div className="shrink-0 text-right">
                    <p className="text-2xl font-extrabold leading-none" style={{ color: GREEN }}>$190.000</p>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-white/10 pt-3 text-sm">
                  ...
                  <span className="rounded-md px-2 py-0.5 text-xs font-bold" style={{ backgroundColor: "rgba(16,185,129,0.22)", color: GREEN }}>Ahorras $50.000</span>
                </div>
              </div>

              <div className="mb-6 rounded-xl px-4 py-3 text-sm" style={{ backgroundColor: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.25)", color: "rgba(255,255,255,0.85)" }}>
          ...
          <div className="hidden lg:block relative h-full flex-1 overflow-hidden">
            <div ref={imgYRef} className="absolute inset-0" style={{ willChange: "transform" }}>
              <Image src={`${BP}/movilidad3.webp`} alt="Movilidad Articular" fill className="object-cover" sizes="50vw" />
            </div>
